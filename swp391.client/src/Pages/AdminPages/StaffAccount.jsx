...
function VetAccount() {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          'https://localhost:7206/api/account-management/roles/4/accounts',
          {
            method: 'GET',
            credentials: 'include',
          }
  ...
  const lockAccount = async (acc) => {
    try {
      const response = await fetch(
        `https://localhost:7206/api/account-management/accounts/lock-account/${acc.accountId}`,
        {
          method: 'PATCH',
          credentials: 'include',
        }
  ...
  const unlockAccount = async (acc) => {
    try {
      const response = await fetch(
        `https://localhost:7206/api/account-management/accounts/unlock-account/${acc.accountId}`,
        {
          method: 'PATCH',
          credentials: 'include',
        }
                ...
                {filteredAccounts
                  ...
                  .map((acc) => (
                    <TableRow hover role="checkbox" tabIndex={-1} key={acc.id}>
                      ...
                      <TableCell>
                        <MDBBadge
                          color={acc.isDisabled ? 'danger' : 'success'}
                          pill
                        >
